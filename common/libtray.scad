...
module DiagonalGrid(unitType, unitSize, gridSize = [ 1, 1 ], noMargin=false) {
  base_num = BaseNum(unitType, unitSize);
  base_size = BaseSize(unitType);

  tm = [
    (base_size[0] * (base_num[0] - gridSize[0]) / (gridSize[0] + (noMargin ? -1 : 1))),
    (base_size[1] * (base_num[1] - gridSize[1]) / (gridSize[1] + (noMargin ? -1 : 1))),
  ];

  spacing = base_size + tm;

  translate(noMargin ? [0, 0] : tm) {
